...
side=worn_panel(64,128,(156,119,41)); dr=ImageDraw.Draw(side)
cream=worn_panel(64,38,(160,125,47))
...
center(dr,(32,54),'ICE COLD',font(7),(80,28,19));center(dr,(32,63),'DOUBLE TAP',font(5),(80,28,19))
save_tex('sides',side)
...
tile('red',(130,2,189,61),(156,119,41))
tile('ivory',(194,2,253,61),(170,133,54))
...
tile('badge',(2,66,125,189),(101,23,20))
# Circular red enamel sign: ivory shield and medical cross, plus a small bullet accent.
d.ellipse((5,69,122,186),fill=(121,30,25),outline=(193,178,137),width=4)
d.ellipse((12,76,115,179),outline=(68,25,21),width=2)
d.polygon([(34,101),(93,101),(91,139),(64,161),(36,139)],fill=(222,208,169))
d.rectangle((57,108,70,147),fill=(126,29,25));d.rectangle((44,121,83,134),fill=(126,29,25))
d.polygon([(48,148),(76,124),(82,132),(55,156)],fill=(112,101,76))
d.polygon([(76,124),(84,124),(82,132)],fill=(213,189,124))
center(d,(64,83),'DOUBLE TAP',font(10),(226,209,166))
center(d,(64,166),'FORTIFIED',font(8),(226,209,166))
for i in range(130):
 x=int(rng.integers(8,121));y=int(rng.integers(72,184))
 if ((x-64)**2+(y-128)**2)>45**2:d.point((x,y),fill=(92,67,48))
...
d.rectangle((195,107,252,157),fill=(192,181,145));center(d,(224,115),'DOUBLE',font(8),(108,31,25));center(d,(224,127),'TAP',font(9),(108,31,25));d.rectangle((219,144,228,153),fill=(106,30,25))
...
tile('label',(2,196,125,253),(181,169,131))
center(d,(64,200),'AUTHORIZED PERSONNEL',font(8),(44,45,36));center(d,(64,214),'ROOT BEER / KEEP COLD',font(8),(71,39,26));center(d,(64,235),'DT-01     110V / 60Hz',font(8),(44,45,36))
...
box('waist_upper_trim',-20,20,-14.7,-13.9,42.2,43,'metal')
box('waist_lower_trim',-20,20,-14.7,-13.9,29.8,30.6,'metal')
...
cylinder_z('display_bottle',-9,-16.3,[(32.1,.85),(36.4,.85),(37.3,.4),(38.6,.4)],8)
...
smd.append('end');(SRC/'doubletap_reference.smd').write_text('\n'.join(smd)+'\n')
(SRC/'idle.smd').write_text('version 1\nnodes\n0 "root" -1\nend\nskeleton\ntime 0\n0 0 0 0 0 0 0\ntime 1\n0 0 0 0 0 0 0\nend\n')
(SRC/'doubletap.qc').write_text('''// Original Half-Life inspired DoubleTap prop. Units match GoldSrc.
$modelname "../svencoop_addon/models/zombies/doubletap_hl1.mdl"
$cd "."
$cdtexture "textures"
$scale 1.0
$body "body" "doubletap_reference"
$sequence "idle" "idle" fps 1 loop
$bbox -22 -20 0 22 14 77
$cbox -22 -20 0 22 14 77
$eyeposition 0 -16 48
''')
(OUT/'svencoop_addon/models/zombies').mkdir(parents=True,exist_ok=True)
obj=['# Original HL1-inspired DoubleTap; Z up; front is -Y; dimensions in GoldSrc units','mtllib doubletap.mtl']
idx=1
for t in tris:
...
(SRC/'doubletap.obj').write_text('\n'.join(obj)+'\n')
(SRC/'doubletap.mtl').write_text('\n'.join(f'newmtl {m}\nKa 0.4 0.4 0.4\nKd 1 1 1\nKs 0 0 0\nd 1\nillum 1\nmap_Kd textures/{m}.png\n' for m in materials))
...
gltf={'asset':{'version':'2.0','generator':'Original DoubleTap mesh builder'},'scene':0,'scenes':[{'nodes':[0]}],'nodes':[{'mesh':0,'name':'DoubleTap_HL1'}],'meshes':[{'primitives':primitives}],'materials':mats,'textures':textures,'images':images,'samplers':[{'magFilter':9728,'minFilter':9728,'wrapS':33071,'wrapT':33071}],'buffers':[{'byteLength':len(blob)}],'bufferViews':views,'accessors':access}
...
(OUT/'doubletap_hl1.glb').write_bytes(glb)
